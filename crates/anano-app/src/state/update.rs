...
//!
//! 发布产物的命名约定在 `.github/workflows/release.yml`：`Anano-<os>-<arch>.<ext>` + `SHA256SUMS` +
...
use anano_core::model::{AppSettings, UpdateSourcePref};
...
pub const REPO_NAME: &str = "Anano";
/// 发布页：自动更新不可用（开发构建、未支持的平台、安装失败）时给用户的退路。
pub const RELEASES_URL: &str = "https://github.com/finch-xu/Anano/releases";
/// 中国大陆镜像的静态 manifest（阿里云 OSS，release.yml 的 mirror-to-oss job 生成并上传）。
/// 桶里是扁平覆盖布局：产物直接躺在 `Anano/` 下，每次发版原地覆盖，只保留最新版。
pub const MIRROR_MANIFEST_URL: &str = "https://d.mirror.catonthe.top/Anano/latest.json";
...
/// 写入方在 `scripts/package-windows.ps1`，打包进 MSI 的声明在 `resources/windows/Anano.wxs`。
...
fn download_dir() -> PathBuf {
    std::env::temp_dir().join("anano-updates")
}
...
pub fn install(cx: &mut App) {
    ...
    let github = GitHubSource::new(REPO_OWNER, REPO_NAME)
        ...
        .include_prereleases(std::env::var_os("ANANO_UPDATE_PRERELEASE").is_some());
...
/// `force`（环境变量 `ANANO_UPDATE_CHECK`）让开发者也能在 `cargo run` 下试。
...
pub fn schedule_launch_check(cx: &mut App) {
    ...
    let force = std::env::var_os("ANANO_UPDATE_CHECK").is_some();
...
mod tests {
    ...
    fn auto_source_follows_ui_language_and_explicit_choice_wins() {
        use anano_core::model::UpdateSourcePref::{Auto, ChinaMirror, Global};
            ...
            fn fetch_latest(&self) -> gpui_updater::Result<gpui_updater::Release> {
                Ok(gpui_updater::Release {
                    ...
                    asset: gpui_updater::Asset {
                        name: "Anano-test".into(),
                        url: format!("https://{}/Anano-test", self.0),
                        size: 0,
                    },
                    signature: None,
                    signature_url: None,
    ...
    fn mirror_manifest_url_is_pinned_to_the_oss_domain() {
        assert_eq!(
            MIRROR_MANIFEST_URL,
            "https://d.mirror.catonthe.top/Anano/latest.json"
        );
    }
