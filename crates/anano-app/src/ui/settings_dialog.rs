...
use anano_core::model::{EDITOR_FONT_SIZE_RANGE, LanguagePref, ThemePref, UpdateSourcePref};
                    ...
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                .gap_3()
                                .items_center()
                                // 多色 logo 走 img()：svg() 是单色蒙版，按主题挑明暗对应那份
                                .child(img(logo_path(cx)).size(px(LOGO_SIZE)).flex_none())
                                .child(
                                    v_flex()
                                        .min_w_0()
                                        .gap_0p5()
