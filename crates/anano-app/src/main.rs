...
// 界面文案：crates/anano-app/locales/app.yml；找不到当前语言的 key 时退回英文
rust_i18n::i18n!("locales", fallback = "en");

use anano_core::store::{Layout, Loaded, Store, StoreError, load_all};
...
actions!(
    anano,
    [OpenRepoAction, CloseTab, ToggleSidebar, OpenSettings]
);
...
fn main() {
    ...
    gpui_kit::platform::application()
        ...
        .run(|cx| {
            ...
            // 清掉上次更新留下的安装包与旧可执行文件（HTTP 响应体落盘的清扫随 body 模块一起移除了）
            ...
            cx.bind_keys([
                KeyBinding::new(&primary("o"), OpenRepoAction, None),
            ...
            cx.spawn(async move |cx| {
                ...
                let opened_window = cx.update(|cx| {
                    install(cx, opened);
                    // 设置在开窗前生效：编辑器字号与界面语言都要在第一帧就是用户的值
