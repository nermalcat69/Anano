...
mod win_resources {
    ...
    ///
    /// VERSIONINFO 让任务管理器与文件属性面板显示「Anano」而不是「anano.exe」。
    pub fn embed() {
        ...
        let icon =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("resources/windows/anano.ico");
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "InternalName", "anano\0"
            VALUE "LegalCopyright", "Copyright (c) 2026 {publisher}\0"
            VALUE "OriginalFilename", "anano.exe\0"
...
"#
        ...
        let rc_path = std::path::Path::new(&out_dir).join("anano.rc");
