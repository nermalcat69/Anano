...
pub use disk::{
    DRAFTS_DIR, Layout, LoadError, Loaded, REPOS_FILE, REQUESTS_DIR, SETTINGS_FILE, WORKSPACE_FILE,
...
impl Store {
    /// 平台数据目录（spec §9.2）：macOS `~/Library/Application Support/Anano`，
    /// Linux `$XDG_DATA_HOME/anano`（默认 `~/.local/share/anano`），Windows `%APPDATA%\Anano\data`。
    pub fn default_root() -> Option<PathBuf> {
        directories::ProjectDirs::from("", "", "Anano").map(|dirs| dirs.data_dir().to_path_buf())
    }
    ...
    pub fn write_repo_list(&self, repos: Vec<RepoEntry>) {
        let path = self.layout.repos_path();
        self.writer
            .write(path, move || codec::encode(&RepoListFile { repos }));
    }
...
mod tests {
    ...
    fn repo_list_round_trips() {
        ...
        let repo = crate::git::RepoEntry::new(std::path::PathBuf::from("/tmp/x"));
        store.write_repo_list(vec![repo.clone()]);
        assert!(store.flush());
        assert_eq!(store.load_all().repos, vec![repo]);
    }

    #[cfg(not(windows))] // Windows 下是 %APPDATA%\Anano\data，末段为 data
    #[test]
    fn default_root_ends_with_anano() {
        let root = Store::default_root().expect("home dir available");
        let name = root.file_name().unwrap().to_str().unwrap();
        assert!(name.eq_ignore_ascii_case("anano"), "{name}");
    }
