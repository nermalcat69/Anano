...
/// 已打开仓库列表（spec Phase 1）：单个文件，与 `workspace.json` 同一套写法。
pub const REPOS_FILE: &str = "repos.json";
/// 可写性探测文件：创建后立即删除。
const PROBE_FILE: &str = ".write-probe";

/// `repos.json` 的落盘形状：`encode`/`decode` 要求顶层是 JSON 对象，`Vec<RepoEntry>`
/// 本身序列化成数组，套一层字段就是对象了。
#[derive(Debug, Default, serde::Serialize, serde::Deserialize)]
pub(crate) struct RepoListFile {
    #[serde(default)]
...
pub struct Loaded {
    ...
    /// 上次打开的仓库列表（phase 1）；没有 `repos.json` 时为空，不算错误。
    pub repos: Vec<RepoEntry>,
    pub errors: Vec<LoadError>,
}

/// 一次性读取全部文件（spec §9.4）。不存在的目录 / 文件视为空；损坏文件改名隔离并继续。
pub fn load_all(layout: &Layout) -> Loaded {
    ...
    let repos = load_file::<RepoListFile>(&layout.repos_path(), &mut errors)
        .map(|f| f.repos)
        .unwrap_or_default();
    Loaded {
        workspace,
...
mod tests {
    ...
    fn paths_follow_spec_layout() {
        let layout = Layout::new(PathBuf::from("/data/Anano"));
        let id = Ulid::generate();
        assert_eq!(
            layout.workspace_path(),
            PathBuf::from("/data/Anano/workspace.json")
        );
        assert_eq!(
            layout.request_path(id),
            PathBuf::from(format!("/data/Anano/requests/{id}.json"))
        );
        assert_eq!(
            layout.draft_path(id),
            PathBuf::from(format!("/data/Anano/drafts/{id}.json"))
        );
    }
