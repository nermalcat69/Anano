#!/usr/bin/env bash
# 把 anano 打成 macOS 的 Anano.app 与 dmg：构建 → 组装 .app → 签名 →（可选）公证并 staple .app
...
#
# 产物：$OUT_DIR/$ARCH_LABEL/Anano.app 与 $OUT_DIR/Anano-macos-$ARCH_LABEL.dmg
#
...
resources="crates/anano-app/resources/macos"
app_name="Anano"
bin_name="anano"
...
version="$(cargo metadata --no-deps --format-version 1 \
  | python3 -c 'import json,sys; m=json.load(sys.stdin); print(next(p["version"] for p in m["packages"] if p["name"]=="anano-app"))')"
[ -n "$version" ] || die "读不到 anano-app 的版本号"
...
if [ "$SKIP_BUILD" != "1" ]; then
  ...
  cargo build --release --locked -p anano-app --target "$TARGET"
fi
