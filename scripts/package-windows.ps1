...
#
#   $OUT_DIR/Anano-windows-$ARCH_LABEL.exe   免安装单文件（名字不能改：老版本的更新器按它匹配）
#   $OUT_DIR/Anano-windows-$ARCH_LABEL.msi   per-user 安装包（WiX v6，装到 %LOCALAPPDATA%\Programs\Anano）
#
# 环境变量：
...
$binary = Join-Path $repoRoot 'target\release\anano.exe'
...
$portable = Join-Path $outDir "Anano-windows-$archLabel.exe"
...
# 版本从 cargo metadata 拿，和 bundle-macos.sh 同源（都读 anano-app 这个包）
...
$version = ($metaJson | ConvertFrom-Json).packages |
    Where-Object { $_.name -eq 'anano-app' } |
    Select-Object -ExpandProperty version
if (-not $version) { throw 'cargo metadata 里找不到 anano-app 的版本' }
...
Copy-Item -LiteralPath $binary -Destination (Join-Path $payloadDir 'anano.exe') -Force
...
$wxs = Join-Path $repoRoot 'crates\anano-app\resources\windows\Anano.wxs'
$icon = Join-Path $repoRoot 'crates\anano-app\resources\windows\anano.ico'
...
$msi = Join-Path $outDir "Anano-windows-$archLabel.msi"
wix build `
    -arch $wixArch `
