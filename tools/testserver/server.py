...
提供一组「难伺候」的 HTTP 接口，用来手工验证 Anano 的慢响应处理、
...
ROUTES_DOC = [
    ...
    },
    {
        ...
        "desc": "模拟 OpenAI Chat Completions 或 Anthropic Messages 的 SSE 事件流："
                ...
                "配 Anano 的流式模板测「收到就展示」、delta 拼装与 TTFT/token 统计。",
        "params": [
    ...
    },
    {
        "path": "/mcp",
        "name": "MCP 服务器",
        "desc": "最小 MCP（Model Context Protocol）端点，配 Anano 侧栏的 MCP 模板，"
...
点接口名直接在浏览器里打开，点「复制」把完整 URL 拿去粘到 Anano。</p>
"""

_INDEX_TAIL = """<p class="note">
提示：<code>/big?size=50mb</code> 这类链接在浏览器里点开会真的下载 50 MB，拿去 Anano 里测更合适。<br>
...
class TestHandler(BaseHTTPRequestHandler):
    ...
    server_version = "AnanoTestServer/1.0"
    sys_version = ""
    ...
    # 所有 method 走同一套处理，这样在 Anano 里换 method 手测时不用换 URL。
    ...
    def ep_slow(self):
        ...
        if chunks is not None:
            self.start(200, content_type_for(kind), size,
                       extra=[("X-Anano-Delay-Seconds", str(delay))])
    ...
    def ep_big(self):
        ...
        extra = [("X-Anano-Body-Bytes", str(size))]
        if chunked:
    ...
    def ep_stream(self):
        ...
        self.start(200, "text/plain; charset=utf-8", chunked=True, extra=[
            ("X-Anano-Total-Chunks", str(count)),
            ("X-Anano-Chunk-Size", str(chunk_size)),
            ("X-Anano-Total-Bytes", str(count * chunk_size)),
        ])
    ...
    def ep_llm(self):
        ...
        事件序列，逐事件 flush，末尾带 usage。配 Anano 的流式模板与 SSE 视图手测。"""
    ...
    def ep_abort(self):
        ...
        self.start(200, "application/json; charset=utf-8", declare,
                   extra=[("X-Anano-Will-Abort-After", str(after))])
        if not self._is_head():
            sent = 0
            for chunk in chunks:
    ...
    def ep_headers(self):
        ...
        extra = [("X-Anano-Test-%03d" % i, _filler(size)) for i in range(1, count + 1)]
        self.send_json(200, {
            "endpoint": "/headers",
            "header_count": count,
            "value_bytes": size,
            "total_header_bytes": count * (size + len("X-Anano-Test-000: \r\n")),
        }, extra=extra)
    ...
    def ep_mcp(self):
        """最小 MCP（Model Context Protocol）服务器，配 Anano 的 MCP 模板。
    ...
    _MCP_SERVER_INFO = {"name": "Anano TestServer", "version": "1.0"}
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Anano 本地测试 server：慢响应、超大响应体、流式滴流、错误码与异常。")
